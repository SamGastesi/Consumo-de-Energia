...
def total_consumo_por_planta_ciudad(planta, ciudad):
    ...
    if ciudad not in consumo_energia[planta].keys():
        return 'La planta ' + planta + ' no proporciona energía a la ciudad de ' + ciudad
...
def ciudad_plantas():
    for llaves in plantas:
        ...
        if ciudad not in plantas:
            print ('La ciudad ingresada no tiene con registros')
            break
...
while op != 0:
    ...
    while op == 1:
        planta = input('Digite el nombre de la planta (Coca Codo Sinclair, Sopladora): ')
        ...
        if type(total) == int:
            ...
            print(total)
        print('¡RETORNANDO A EL MENU PRINCIPAL!')
        break
    while op == 2:
        ciudad =  input('Digite el nombre de la ciudad: ')
        ciudad_plantas()
